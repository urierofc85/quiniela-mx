...
export default function Survivor() {
  ...
  const cargarDatos = async () => {
    // Verifica automáticamente si alguna jornada ya venció
  ...
  // Lista única de equipos para toda la temporada
  ...
  const cargarUsoEquipos = async () => {
    ...
    setUsoEquipos(
      Object.entries(conteo).map(([equipo, usos]) => ({
        equipo,
        usos,
      }))
    );
  ...
  const cargarHistorial = async () => {
    ...
    const procesado = (selecciones || []).map((item) => {
      ...
      return {
        ...item,
        nombreJornada:
          jornada?.nombre || `Jornada ${item.jornada_id}`,
        puntos,
        resultado,
      };
    });
    ...
    // Verificar si ya venció la jornada
    await verificarJornadas();
    ...
    const horaMexico = await obtenerHoraMexico();
    const fechaLimite = new Date(jornada.fecha_limite);
    ...
    // Validar máximo 3 usos del mismo equipo
    ...
    const usos =
      actual?.equipo === equipoSeleccionado
        ? (count || 0) - 1
        : (count || 0);

    if (usos >= 3) {
      alert(
        `Ya utilizaste ${equipoSeleccionado} tres veces.`
      );
      return;
    }

    // Eliminar selección previa de la jornada
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      ...
      {jornadaActiva ? (
        ...
        <div className="bg-yellow-100 border border-yellow-400 p-4 rounded mb-8">
